...
@if($loop->depth <= 7)
    ...
    <?php
            ...
            if($payment){
                $userIncome = DB::table('user_incomes')->where('child_id', $child->id)->where('user_id', $joiner->id)->where('plan', 10500)->first();
                // dd($userIncome);
                if(empty($userIncome))
                {
        ...
        @if(count($child->childs))
            @include('admin.company-tree.income',['user_childs' => $child->user_childs])
        @endif
    </li>
@endif
@endforeach
